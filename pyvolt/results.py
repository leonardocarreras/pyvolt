...
class Results():
    ...
    def get_voltages(self, pu=True):
        ...
        """
        voltages = np.zeros(len(self.nodes), dtype=complex_)
        if pu == True:
            for node in self.nodes:
    ...
    def get_branch_powers(self, pu=True):
        ...
        """
        #branch_powers = np.zeros(len(self.branches), dtype=complex_)
        branch_powers = []
        if pu == True:
            for branch in self.branches:
    ...
    def get_Iinj(self, pu=True):
        ...
        - if pu==True --> voltages are expressed as per-unit
        """
        Iinj = np.zeros(len(self.nodes), dtype=complex_)
        if pu == True:
            for node in self.nodes:
    ...
    def get_Sinj(self, pu=True):
        ...
        - if pu==True --> voltages are expressed as per-unit
        """
        Sinj = np.zeros(len(self.nodes), dtype=complex_)
        if pu == True:
            for node in self.nodes:
    ...
    def getI(self, pu=True):
        ...
        - if pu==True --> voltages are expressed as per-unit
        """
        I = np.zeros(len(self.branches), dtype=complex_)
        if pu == True:
    ...
    def get_S1(self, pu=True):
        ...
        - if pu==True --> voltages are expressed as per-unit
        """
        S1 = np.zeros(len(self.branches), dtype=complex_)
        if pu == True:
            for branch_idx in range(len(self.branches)):
    ...
    def get_S2(self, pu=True):
        ...
        - if pu==True --> voltages are expressed as per-unit
        """
        S2 = np.zeros(len(self.branches), dtype=complex_)
        if pu == True:
            for branch_idx in range(len(self.branches)):
